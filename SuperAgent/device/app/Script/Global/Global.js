...
function SetSessionConstants() {
  ...
  $.sessionConst.Add("GPSTrackDistance", GPSTrackDistance.ExecuteScalar());
	...
	$.sessionConst.Add("DayPlanVisitCount", countDayPlanEnd);
...
function SetGps() {
	if (!$.sessionConst.GPSTrack){
		...
		//Dialog.Message("Stop 152");
	}
	//Dialog.Message("153");

	GPSTracking.Stop();
	//Dialog.Message("Stop 157");
	...
	if ($.workConst.GPSStarted || $.sessionConst.GPSPredefined){
		...
		GPSTracking.MinDistance = parseInt($.sessionConst.GPSTrackDistance);
