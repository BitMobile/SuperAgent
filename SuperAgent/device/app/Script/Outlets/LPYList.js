...
function GetPlans(outlet, sr) {
	var q = new Query("select PW.Id, PWW.Description from Catalog_Outlet_PlacesWork PW LEFT JOIN Catalog_Outlet PWW ON PW.PlaceWork=PWW.Id WHERE PW.Ref=@outlet");
	q.AddParameter("outlet", outlet);
//	Dialog.Debug(outlet);
...
function CreatePlan(outlet, plan, planDate) {
	Dialogs.Choose(plan, "PlanDate", null, PlanHandler); //(header, planDate, PlanHandler, [ outlet, plan ]);
}
