...
function GetOutlets(searchText) {
	...
	// if ($.sessionConst.encashEnabled){
		outletStatus = "CASE WHEN ((O.OverdueObligation != '' AND O.OverdueObligation != '0') AND ((O.Streams <= 0) OR (O.Streams > 0 AND O.NormPassability >= 90))) THEN 0 " +
		"WHEN ((O.OverdueObligation = '' OR O.OverdueObligation = '0') AND (O.Streams > 0 AND O.NormPassability < 90)) THEN 1 " +
		"WHEN ((O.OverdueObligation != '' AND O.OverdueObligation != '0') AND (O.Streams > 0 AND O.NormPassability < 90)) THEN 2 ELSE 3 END AS OutletStatus";
	// }
	// else
...
function CoordsChecked(visit) {
	...
	Dialog.Debug(GPS.Accuracy);
	if (!(IsNullOrEmpty(GPS.Accuracy))){
