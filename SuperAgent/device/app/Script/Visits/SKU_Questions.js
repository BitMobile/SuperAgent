...
/////////////////////
			...
			"WHERE SKU=S.SKU AND (Answer='Yes' OR Answer='Да'))) " +
		 "OR S.SKU=@sku AND S.Single=@single AND (S.SKU=@sku AND S.Single=@single  " +
		 	"  AND S.ParentQuestion IN (SELECT Question FROM USR_SKUQuestions WHERE SKU=S.SKU AND (AnswerId <>'' AND Single=@single)) " +
		 	"  AND S.VersionAnswerValueQuestion IN (SELECT AnswerId FROM USR_SKUQuestions WHERE SKU=S.SKU AND (AnswerId <>'' AND Single=@single))) " +
