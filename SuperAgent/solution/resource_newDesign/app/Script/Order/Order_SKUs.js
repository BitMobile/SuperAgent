...
function GetSKUAndGroups(searchText, priceList) {
    if (String.IsNullOrEmpty(searchText)) {
        var query = new Query();
        query.Text = "SELECT S.Id, S.Description, PL.Price, S.CommonStock, G.Description AS GroupDescription FROM Catalog_SKU S JOIN Catalog_SKUGroup G ON G.Id = S.Owner JOIN Document_PriceList_Prices PL ON PL.SKU = S.Id WHERE S.CommonStock>0 AND PL.Ref = @Ref ORDER BY G.Description, S.Description LIMIT 100";
...
function GetParentGroups() {
    ...
    var q = new Query("SELECT Id, Description, Parent FROM Catalog_SKUGroup WHERE Parent IS NULL ORDER BY S.Description");
    Dialog.Debug(q.ExecuteCount());
    var c = q.Execute();
    while (c.Next()){
        Dialog.Debug(c.Parent);
    }
    return q.Execute();
}
...
function GetBrands() {
    ...
    var q = new Query("SELECT B.Id, B.Description FROM Catalog_Brands B JOIN Catalog_SKU S ON B.Id=S.Brand ORDER BY B.Description");
    return q.Execute();
}