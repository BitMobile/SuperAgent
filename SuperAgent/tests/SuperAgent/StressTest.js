...
function Sync() {
	...
	if (timer == "180" || timer > "180") {
		Console
				.WriteLine(Device.Click("btnSync")
...
function CheckEmptyScreens() {
	...
	case "Visit_Questions.xml":
		...
		Console.WriteLine(TextCheck(
				"grScrollView.Controls[10].Controls[0].Controls[0]", "String"
						+ n)); // Каталог товара есть в магазине" Question
								// string

		Console.WriteLine(TextCheck(
				"grScrollView.Controls[7].Controls[0].Controls[0]", "123546"
						+ n)); // ,"Доля полки %" //Question int

		Console.WriteLine(CheckValue(
				"grScrollView.Controls[12].Controls[0].Controls[0].Text",
				"Набор рекомендуемых  к размещению  POSM установлен"));// ,"Набор
																		// рекомендуемых
																		// к
																		// размещению
																		// POSM
		...
		var result = Device
				.Click("grScrollView.Controls[13].Controls[0].Controls[0]");

		Console.WriteLine(TextCheck(
				"grScrollView.Controls[19].Controls[0].Controls[0]", "123.6"
						+ n)); // Стандарт по доле полки относительно
								// конкурентов выполнен")); Question (Decimal)
	...
	case "Outlet.xml":
		Console.WriteLine("Определился Outlet.xml");
	}
}
...
function main() {
	...
	while (i < 500) {
		...
		if (result == "Для этой торговой точки нет прайс-листов") {
			...
			// var result=Device.SetFocus("grScrollView.Controls[0]");
			// //Checking groups
		...
		Console.WriteLine(TextCheck(
				"grScrollView.Controls[0].Controls[1].Controls[0]", n)); // Quantity
