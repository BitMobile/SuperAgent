...
function main() {
	...
	if (result == "OrderList.xml") {
		...
		Console.Terminate(result != "OrderList.xml",
	...
	var GetCurrentScreenName = Device.GetValue("context.CurrentScreen.Name");
	if (GetCurrentScreenName == "SKU_Image.xml") {
		var result = Device.Click("btnBack");
		Console.WriteLine(result);
	} else {
		Console.Terminate(GetCurrentScreenName != "SKU_Image.xml",
				"Кнопка Вперед не заблокирована!");
	}
	...
	var CurrentScreenName = Device.GetValue("context.CurrentScreen.Name");
	if (CurrentScreenName == "Stock_SKUs.xml") {
		var result = Device.Click("btnBack");
		Console.WriteLine(result);
	} else {
		Console.Terminate(CurrentScreenName != "Stock_SKUs.xml",
				"Кнопка Вперед не заблокирована!");
	}
	...
	var result = Device.GetValue("context.CurrentScreen.Name");
	Console.Terminate(result != "Main.xml",
			"Кнопка Настройки не заблокирована!");
