...
function RunStopWatch(type, path, value) {
	switch (type) {
	case "screenOpenning":
		Device.Click(path);
		Stopwatch.Start();
		...
		} while (CurrScreen != "Order_SKUs.xml");
		var result = Stopwatch.Stop();
		Console.WriteLine(result.TotalSeconds + value + "  loading time");
	case "Search":

		Stopwatch.Start();
		do {
			var SerchResult = Device
					.GetValue("grScrollView.Controls[2].Controls[0].Controls[0].Text");
		} while (SearchResult != "Order_SKUs.xml");
		var result = Stopwatch.Stop();
		Console.WriteLine(result.TotalSeconds + StopScreen + "  loading time");

		// var result =
		// CheckValue("grScrollView.Controls[2].Controls[0].Controls[0].Text",
		// "Гвоздь 10ка");
		// if (result=="True") {
		// var result = Stopwatch.Stop();
